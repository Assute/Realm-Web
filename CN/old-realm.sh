...
show_menu() {
    ...
    echo " ———————————— realm版本v2.7.0 ————————————"
    echo "     修改by：Azimi    修改日期：2025/1/17"
    echo "     修改内容：1.修改查看转发规则内容更加清晰"
    echo "               2.添加/删除规则后自动重启服务"
    echo "               3.更新realm版本至2.7.0"
    echo "     "
    echo "     (1)更新脚本请先删除脚本 rm realm.sh"
    echo "     (2)安装前请先更新系统软件包，缺少命令可能无法安装"
    echo "     (3)如果启动失败请检查 /root/realm/config.toml下有无多余配置或者卸载后重新配置"
    echo "     (4)该脚本只在debian系统下测试，未做其他系统适配，安装命令有别，可能无法启动。如若遇到问题，请自行解决"
    echo "     PS:没多少时间做系统适配，我尽力而为"
echo "     仓库：https://github.com/Assute/Realm-Web"
    echo " "
    echo "——————————————————"
    echo " 1. 安装 realm"
    echo "——————————————————"
...
deploy_realm() {
    mkdir -p /root/realm
    cd /root/realm
    wget -O realm.tar.gz https://mirror.ghproxy.com/https://github.com/zhboner/realm/releases/download/v2.7.0/realm-x86_64-unknown-linux-gnu.tar.gz
